...
items = zapi.item.get(hostids=hostids,output='extend',search={'key_':args.key}, monitored=True, selectHosts=['name','host'])
if items:
    # Sort on hostname
    ...
    header=['name','host','puerto'] # CSV header
    for day in datelist:
    ...
    for item in items:
        row = [item['hosts'][0]['name'],item['hosts'][0]['host'],item["name"]]
        trends = None
