...
from proc_reporte_gpon import f_parsear_inventario
...
def ejecutar_sql(sql,tipo_sql):
    try:
        ...
        print( f'[{datetime.now()}] Connected to DB: {tipo_sql}')
        # Create cursor and execute Load SQL
        cursor = mydb.cursor()
        cursor.execute(sql)
        print(f'[{datetime.now()}] Succuessfully loaded: {cursor.rowcount} affected rows ')
        mydb.commit()
        mydb.cursor().close()
        mydb.close()
...
#            Parsear datos         #
#==================================#
#llamo a la funcion f_parsear_inventario ubicada en el archivo ./proc_reporte_gpon.py
# Toma el archivo guardado en /var/lib/reportes-zabbix/reporte_tlk/PLN245_procesado.TXT
# Deja el archivo en /var/lib/reportes-zabbix/reporte_tlk/PLN245_parseado.csv

f_parsear_inventario()
...
sql_cargar_PLN = "LOAD DATA INFILE \'/var/lib/reportes-zabbix/reporte_tlk/PLN245_parseado.csv\' INTO TABLE t_reporte_puertos_telelink FIELDS TERMINATED BY \',\' ENCLOSED BY \'\"\' LINES TERMINATED BY \'\\r\\n\' ;"
